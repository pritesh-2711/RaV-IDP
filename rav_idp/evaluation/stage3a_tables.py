...
def _tatr_table_record(image_bytes: bytes) -> dict:
    ...
    for row_idx, (ry_center, (rx0, ry0, rx1, ry1)) in enumerate(rows):
        ...
        for col_idx, (_, (cx0, cy0, cx1, cy1)) in enumerate(cols):
            # Cell = intersection of row horizontal band × column vertical band
            cell_x0 = max(rx0, cx0)
            cell_y0 = max(ry0, cy0)
            cell_x1 = min(rx1, cx1)
            cell_y1 = min(ry1, cy1)
...
def _derive_ground_truth(annotation: dict) -> TableGroundTruth:
    ...
    if ordered_cells and row_centers:
        header_cells = [
            cell for cell in ordered_cells if _nearest_cluster(float(cell["bbox"][1]), row_centers) == header_row_index
        ]
...
def _dataframe_signature(frame_json: str) -> tuple[int, int, list[str], list[str]]:
    ...
    cell_texts = [_normalize_text(str(value)) for row in frame.itertuples(index=False) for value in row if str(value).strip()]
...
def run_table_benchmark(
    ...
    # Over-read candidates so that after the size filter we still reach `limit` samples.
    ...
    for annotation in annotations:
        ...
        # Skip images whose short dimension is below the minimum evaluation threshold.
        # These produce cell crops too small for reliable OCR and introduce noise.
        ...
        fidelity = compare_table(reconstruction.content, region, settings.threshold_table)
    ...
    mean_row_abs_error = sum(abs(record.predicted_rows - record.ground_truth_rows) for record in records) / len(records)
    mean_col_abs_error = sum(abs(record.predicted_cols - record.ground_truth_cols) for record in records) / len(records)
